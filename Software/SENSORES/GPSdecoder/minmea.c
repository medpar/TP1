...
enum minmea_sentence_id minmea_sentence_id(const char *sentence, bool strict)
{
    ...
    if (!minmea_scan(sentence, "t", &type))
        return MINMEA_INVALID;
    unsigned int i;
    for (i = 0; i < countof(sentence_id_map); i++) {
...
int minmea_gettime(struct timespec *ts, const struct minmea_date *date, const struct minmea_time *time_)
{
    ...
    // Convertimos struct tm a time_t manualmente (UTC)
    time_t timestamp;

    // mktime usa la zona horaria local, por eso suele dar problemas con GPS (UTC)
    // Usamos una aproximación simple si no hay timegm
    timestamp = mktime(&tm);
